...
mb <- read.csv("interpolated_counts_3_5_with_metadata_fev1_per_no.csv")
...
useMe<- ! (is.na(Y$Persistent_infection)|is.na(X$Persistent_infection))
X <- X[useMe,]
Y <- Y[useMe,]
...
X <- X[,goodOTUs]
Y <- Y[,goodOTUs[15:length(goodOTUs)]]
X <<- X
Y <<- Y
#oxy.taxa <<- oxy.taxa
 #end with()
