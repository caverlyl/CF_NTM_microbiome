...
source("data_prep_per_yes.R")
library(corrplot)
library(reshape2)
library(igraph)
library(dplyr)
library(fields)
results <- read.csv("PoissonARIMA-Disease-fev1_per_yes.csv")
...
pdf("Persistent_yes_coef_matrix.pdf")
corrplot(as.matrix(mat[,c(1:17)]),addgrid.col = "white",tl.col = "black",  na.label = "square", na.label.col = "gray",is.corr=FALSE,tl.cex=0.7,number.cex=1,method="color")
dev.off()

input <- as.matrix(mat[,c(1:17)])
...
#pdf(file="Persistent_infection_yes_network_ceb.pdf")
#plot(ceb, net,vertex.label.cex=0.4)
#dev.off()

#pdf(file="Persistent_infection_yes_network_ceb_noshade.pdf")
...
E(net)$color = apply(c_scale(range01(weights))[1:77,], 1, function(x) rgb(x[1]/255,x[2]/255,x[3]/255) )
...
#pdf(file="Persistent_infection_yes_network_ceb_edgecolor.pdf")
...
pdf(file="Persistent_infection_yes_network_ceb_noshade_edgecolor.pdf")
