...
library(dplyr)
library(funModeling)
results <- read.csv("PoissonARIMA-Disease-fev1_dis_yes.csv")
...
meanReads[1:8] <- 1 #animal effects need to have a 1
...
pdf("Disease_yes_coef_matrix.pdf")
...
#pdf(file="NTM_disease_yes_network_ceb.pdf")
#plot(ceb, net,vertex.label.cex=0.4)
#dev.off()

#pdf(file="NTM_disease_yes_network_ceb_noshade.pdf")
...
E(net)$color = apply(c_scale(range01(weights))[1:39,], 1, function(x) rgb(x[1]/255,x[2]/255,x[3]/255) )
...
#pdf(file="NTM_disease_yes_network_ceb_edgecolor.pdf")
#plot(ceb, net,vertex.label.cex=0.4, edge.color=E(net)$color, vertex.color=tester)
#dev.off()


pdf(file="NTM_disease_yes_network_ceb_noshade_edgecolor.pdf")
#plot(net, vertex.color=tester,vertex.frame.color=NA, vertex.label.cex=0.5,vertex.label.dist=0.95,vertex.size=8,vertex.label.degree=pi/2,vertex.label.color="black")
