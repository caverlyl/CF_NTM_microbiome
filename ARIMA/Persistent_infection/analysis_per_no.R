...
library(igraph)
library(dplyr)
results <- read.csv("PoissonARIMA-Disease-fev1_per_no.csv")
...
meanReads[1:14] <- 1 #animal effects need to have a 1
...
pdf("Persistent_no_coef_matrix.pdf")
corrplot(as.matrix(mat[,c(1:16)]),addgrid.col = "white",tl.col = "black",  na.label = "square", na.label.col = "gray",is.corr=FALSE,tl.cex=0.7,number.cex=1,method="color")
dev.off()
...
#pdf(file="Persistent_infection_no_network_ceb.pdf")
#plot(ceb, net,vertex.label.cex=0.4)
#dev.off()

#pdf(file="Persistent_infection_no_network_ceb_noshade.pdf")
...
E(net)$color = apply(c_scale(range01(weights))[1:22,], 1, function(x) rgb(x[1]/255,x[2]/255,x[3]/255) )
...
#pdf(file="Persistent_infection_no_network_ceb_edgecolor.pdf")
...
pdf(file="Persistent_infection_no_network_ceb_noshade_edgecolor.pdf")
